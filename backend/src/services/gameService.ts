...
export class GameService {
  ...
  async joinGame(gameCode: string, playerNickname: string, playerSocketId: string): Promise<IGame> {
    ...
    const existingPlayer = game.players.find((p: { id: string; nickname: string; socketId: string; isCreator: boolean; joinedAt: Date }) => p.nickname === playerNickname);
    if (existingPlayer) {
  ...
  async startGame(gameCode: string, playerId: string): Promise<IGame> {
    ...
    const player = game.players.find((p: { id: string; nickname: string; socketId: string; isCreator: boolean; joinedAt: Date }) => p.id === playerId);
  ...
  async removePlayer(gameCode: string, playerId: string): Promise<IGame | null> {
    ...
    const playerIndex = game.players.findIndex((p: { id: string; nickname: string; socketId: string; isCreator: boolean; joinedAt: Date }) => p.id === playerId);
    if (playerIndex === -1) {
      return game;
    }
